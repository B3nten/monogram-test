...
export function Model(props: GroupProps) {
	...
	useEffect(() => {
		...
		function foo(e: MouseEvent) {
			...
			if (
				typeof group.current?.rotation?.x === 'number' &&
				typeof group.current?.rotation?.y === 'number'
			) {
				group.current.rotation.x = mouseY * 0.3 + 0
				group.current.rotation.y = mouseX * 0.2 - 1.6
			}
		}
